...
assert(originCatalogMeta.documentGenerator === "Coffendi bilingual origin document generator", "Unexpected bilingual document generator");
...
const artworkPath = path.join(
  ...
  "images",
  "catalog",
  `${originCatalogMeta.decorativeArtwork}.png`,
);
const artwork = await loadImage(artworkPath);
const artworkStat = await stat(artworkPath);
assert(artwork.width >= 1500 && artwork.height >= 1000, "Generated catalogue artwork is below its source-resolution floor");
assert(artworkStat.size >= 1_000_000, "Generated catalogue artwork is unexpectedly compressed");
