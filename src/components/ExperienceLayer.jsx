...
function DiscoveryBoot({ error, language, onRetry }) {
  const copy = language === "tr"
    ? error
      ...
      : ["Menşe araması hazırlanıyor", "Kahve kataloğu güvenli biçimde açılıyor."]
    : error
      ? ["Search could not load", "Check your connection and try again.", "Try again"]
      : ["Preparing origin search", "Opening the coffee catalogue without leaving this page."];
  return (
    <div className={`connection-notice discovery-boot ${error ? "is-error" : "is-online"}`} role={error ? "alert" : "status"} aria-live={error ? "assertive" : "polite"} aria-atomic="true">
      <span className="discovery-boot__signal" aria-hidden="true" />
...
const REVEAL_SELECTOR = [
  ...
  ".origin-atlas__header > *",
  ".origin-atlas__controls",
  ".origin-atlas__visual",
  ".origin-atlas__readout",
  ".section-heading",
  ".profile-card",
  ".story-grid > *",
  ".story-steps > li",
...
export default function ExperienceLayer({ language, profiles }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
        ...
        openDiscovery({ trigger: document.activeElement });
  ...
  useEffect(() => {
    ...
    const selector = ".coffee-map,.origin-filter-panel,.origin-page-reader,.origin-constellation__board,.compare-workbench,.inquiry-form";
  ...
  return (
    <>
      ...
      {DiscoveryDeck && discoveryRequest && <DiscoveryDeck language={language} profiles={profiles} openRequest={discoveryRequest} />}
